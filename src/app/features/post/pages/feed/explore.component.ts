...
import { Comment, ExploreItem } from 'src/app/core/models/social.model';
    ...
    <div class="space-y-6 sm:py-4">
      ...
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Discover trending content, amazing photography, and stories from our
          community.
        </p>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        >
          ...
          <div
            *ngIf="!item.imageUrl"
            class="w-full h-full p-4 flex items-center justify-center bg-violet-50 dark:bg-violet-900/20 text-center"
          >
            <span
              class="text-sm font-medium text-slate-700 dark:text-slate-300 line-clamp-4 break-words w-full"
              >{{ item.caption }}</span
            >
          </div>
          ...
          <!-- Hover Overlay -->
          <div
            class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-6 z-10"
          >
        ...
        >
          ...
          <div
            *ngIf="selectedItem()?.imageUrl"
            class="md:w-3/5 bg-slate-950 flex items-center justify-center relative min-h-[300px] md:min-h-0"
          >
            <img
          ...
          <!-- Content Section -->
          <div
            class="p-6 flex flex-col justify-between bg-white dark:bg-slate-800 overflow-y-auto"
            [ngClass]="selectedItem()?.imageUrl ? 'md:w-2/5' : 'md:w-full'"
          >
            ...
            <div class="space-y-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h4
                    class="font-extrabold text-slate-800 dark:text-white hover:underline cursor-pointer"
                  >
              ...
              <div class="bg-slate-50 dark:bg-slate-700/30 rounded-2xl p-4">
                <p
                  class="text-sm text-slate-700 dark:text-slate-200 leading-relaxed"
                >
            ...
            <!-- Real Comments -->
            <div
              class="mt-6 border-t border-slate-100 dark:border-slate-700 pt-4 flex-grow overflow-y-auto space-y-3 max-h-48"
            >
              <div
              ...
              <div
                *ngIf="
                  !isLoadingComments() && selectedItemComments().length === 0
                "
              ...
              <div *ngFor="let c of selectedItemComments()" class="flex gap-2">
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-sm text-slate-700 dark:text-slate-300">
                    <span
                      class="font-bold text-slate-900 dark:text-white mr-1"
                      >{{ c.username }}</span
                    >{{ c.text }}
                  </p>
            ...
            <!-- Stats -->
            <div
              class="border-t border-slate-100 dark:border-slate-700 pt-4 mt-6 flex justify-between items-center"
            >
              <div class="flex items-center gap-2">
                <span
                  class="text-sm font-extrabold text-slate-800 dark:text-white"
                  >{{ selectedItem()?.likes }} likes</span
                >
              </div>
